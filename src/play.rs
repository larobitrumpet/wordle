...
pub fn play() {
    ...
    for _ in 0..6 {
        ...
        update_possibilities(&mut possibilities, &word_guessed, &colors, &mut green, &mut yellow);
        possible_words = update_possible_words(&possibilities, possible_words, &yellow);
        println!("{:?}", possible_words);
        println!("{:?}", possibilities);
        word_guessed = optimal_word(&possibilities, possible_words.clone(), &green, &yellow, true);
